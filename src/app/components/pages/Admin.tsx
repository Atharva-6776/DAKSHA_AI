...
import { CheckCircle, Clock, XCircle, MoreVertical, FileText, Eye, Layers, ScanLine, MapPin } from "lucide-react";
...
export const Admin = () => {
  ...
  const radarData = [
    { subject: "ELA",    value: ai.ela    },
    { subject: "YOLO",   value: ai.yolo   },
    { subject: "OpenCV", value: ai.opencv },
    { subject: "Clean",  value: ai.clean  },
  ];
  ...
  const cityData = useMemo(() => {
    ...
    return Object.values(map)
      .map((d: any) => ({ ...d, rate: d.total > 0 ? Math.round((d.verified / d.total) * 100) : 0 }))
  ...
  const toggleSort = (f: "date" | "status") => {
    if (sortField === f) setSortAsc(!sortAsc);
    else { setSortField(f); setSortAsc(false); }
  };

  const handleExportCSV = () => {
    const headers = ["ID", "Location", "Date", "Status", "ELA", "YOLO", "OpenCV", "Clean"];
    ...
    <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 48 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", paddingBottom: 24, borderBottom: "1px solid rgba(57,63,77,0.4)" }}>
        ...
        <button onClick={handleExportCSV}
          style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 28px", background: "transparent", border: `1px solid rgba(57,63,77,0.6)`, color: C.silver, fontFamily: "'Rajdhani', sans-serif", fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", fontWeight: 600, cursor: "pointer", transition: "all 0.3s" }}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
            ...
            <div style={S.card}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", borderBottom: "1px solid rgba(57,63,77,0.35)" }}>
                {[
                  { n: stats.verified, label: "Verified", color: "#22c55e" },
                  { n: stats.pending,  label: "Pending",  color: C.yellow  },
                  { n: stats.rejected, label: "Rejected", color: C.grey    },
                ].map(({ n, label, color }, i) => (
              ...
              <div style={{ padding: "20px 28px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  ...
                  <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 18, color: C.silver, fontStyle: "italic" }}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    ...
                    <Legend verticalAlign="bottom" height={36} iconType="square"
                      formatter={v => <span style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 11, color: C.silver, letterSpacing: "0.1em" }}>{v}</span>} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
            </div>
              ...
              <div style={{ flex: 1, overflow: "hidden" }}>
                {reports.slice(0, 4).map((r, i) => (
                  <motion.div key={r.id} whileHover={{ x: 4 }}
                    style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 24px", borderBottom: i < 3 ? "1px solid rgba(57,63,77,0.3)" : "none", cursor: "pointer" }}>
                ...
                {reports.length === 0 && (
                  <div style={{ padding: "40px 24px", textAlign: "center", ...S.body }}>No reports yet.</div>
                )}
              </div>
            </div>
          </div>
                    ...
                    {sortedReports.map(r => {
                      ...
                      const Pill = ({ val, label }: { val: number | undefined; label: string }) =>
                        val !== undefined && !isNaN(val) ? (
                          <span style={{ padding: "3px 8px", fontSize: 11, fontFamily: "'Rajdhani', sans-serif", letterSpacing: "0.1em", background: val > 80 ? "rgba(34,197,94,0.1)" : "rgba(254,218,106,0.1)", color: val > 80 ? "#22c55e" : C.yellow, border: `1px solid ${val > 80 ? "rgba(34,197,94,0.2)" : "rgba(254,218,106,0.2)"}` }}>
                            {label}:{val}%
                          </span>
                      ...
                      return (
                        <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                          style={{ borderBottom: "1px solid rgba(57,63,77,0.25)", transition: "background 0.2s" }}
                          ...
                          <td style={{ padding: "14px 16px" }}>
                            <span style={{ padding: "4px 12px", fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", background: r.status === "Verified" ? "rgba(34,197,94,0.1)" : r.status === "Pending" ? "rgba(254,218,106,0.1)" : "rgba(57,63,77,0.3)", color: r.status === "Verified" ? "#22c55e" : r.status === "Pending" ? C.yellow : C.grey, border: `1px solid ${r.status === "Verified" ? "rgba(34,197,94,0.2)" : r.status === "Pending" ? "rgba(254,218,106,0.2)" : "rgba(57,63,77,0.4)"}` }}>
                              {r.status}
                            </span>
                          </td>
