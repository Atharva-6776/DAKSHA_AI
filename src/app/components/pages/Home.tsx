...
export const Home = () => {
  ...
  const gpsCount      = reports.filter(r => r.lat && r.lng).length;
  const gpsRate       = totalReports > 0 ? Math.round((gpsCount / totalReports) * 100) : 100;
    ...
    <div style={{ width: "100%", background: C.dark }}>
      ...
      <div style={{ position: "relative", height: "100vh", overflow: "hidden" }}>
        <AnimatePresence mode="wait">
          <motion.div key={bg} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.5 }}
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 5, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 80px" }}>
          ...
          <div style={{ position: "absolute", left: 60, top: "50%", transform: "translateY(-60%)", fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(180px, 26vw, 360px)", fontWeight: 600, color: "transparent", WebkitTextStroke: "1px rgba(212,212,220,0.04)", lineHeight: 1, userSelect: "none" }}>
          ...
          <motion.h1 initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
            style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(52px, 9vw, 108px)", fontWeight: 300, fontStyle: "italic", color: C.silver, lineHeight: 0.93, margin: "0 0 28px", maxWidth: 680, position: "relative", zIndex: 2 }}>
          ...
          <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }}
            style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 16, color: "rgba(212,212,220,0.5)", lineHeight: 1.9, fontWeight: 300, maxWidth: 460, marginBottom: 48, letterSpacing: "0.025em", position: "relative", zIndex: 2 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.5 }}
            ...
            <Link to="/citizen"
              style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "14px 34px", background: C.yellow, color: C.dark, fontFamily: "'Rajdhani', sans-serif", fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", fontWeight: 700, transition: "background 0.3s" }}
            ...
            <Link to="/summary"
              style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "14px 34px", background: "transparent", color: C.silver, fontFamily: "'Rajdhani', sans-serif", fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", fontWeight: 600, border: "1px solid rgba(212,212,220,0.22)", transition: "all 0.3s" }}
        ...
        <div style={{ position: "absolute", bottom: 44, left: 80, zIndex: 6, display: "flex", gap: 10 }}>
          {bgSlides.map((_, i) => (
            <button key={i} onClick={() => setBg(i)}
              style={{ border: "none", cursor: "pointer", height: 2, borderRadius: 0, width: i === bg ? 40 : 14, background: i === bg ? C.yellow : "rgba(212,212,220,0.2)", transition: "all 0.5s" }} />
          ))}
        </div>

        {/* AI badge */}
        <div style={{ position: "absolute", bottom: 44, right: 80, zIndex: 6, display: "flex", alignItems: "center", gap: 10, padding: "10px 18px", border: "1px solid rgba(254,218,106,0.18)", background: "rgba(20,21,26,0.75)", backdropFilter: "blur(10px)" }}>
      ...
      <div style={{ display: "flex", borderTop: "1px solid rgba(57,63,77,0.5)", borderBottom: "1px solid rgba(57,63,77,0.5)" }}>
        {cards.map((c, i) => (
          <button key={i} onClick={() => setActiveTab(i)}
            style={{ flex: 1, padding: "22px 0", textAlign: "center", fontFamily: "'Rajdhani', sans-serif", fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", fontWeight: 600, cursor: "pointer", border: "none", transition: "all 0.35s", background: activeTab === i ? C.yellow : C.grey, color: activeTab === i ? C.dark : C.silver, borderRight: i < cards.length - 1 ? "1px solid rgba(57,63,77,0.5)" : "none" }}>
            {c.tab}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderTop: "1px solid rgba(57,63,77,0.4)" }}>
        {[
          { num: `${totalReports}+`,  label: "Reports Filed"    },
          { num: "3-Layer",           label: "AI Verification"  },
          { num: `${verifiedCount}`,  label: "Areas Verified"   },
          { num: `${gpsRate}%`,       label: "GPS Verified"     },
        ].map(({ num, label }, i) => (
